...
function main(mode, project_dir)
	if mode == nil then
	...
	if mode ~= 'run' and mode ~= 'build' then
		print"usage: jina build|run <project_path> [gcc_options]"
		return
	end
	
	if not path.isdir(project_dir) then
	...
	for _, dir_path in ipairs(dir.filter(dir.getdirectories(project_dir), "*.jin")) do
		...
		dir.getallfiles(pkg.path):foreach(function (file_path)
			...
			elseif file_path:find"%.p$" then
				...
				if url then
					...
					local url_hash = io.popen('echo -n ' .. url .. ' | md5sum | cut -d " " -f1'):read()
				...
				then
					...
					pkg_table[dep_pkg_id] = { path = dep_pkg_path, dlibs = "", spm = "", dep_out_paths = {} }
				end
			end
		end)
		i = i + 1
	...
	for i = #pkg_id_list, 1, -1 do
		...
		if path.isfile(path.join(pkg.path, "0.jin")) then
			...
			if mode == 'run' then os.execute("LD_LIBRARY_PATH=. " .. out_executable_path) end
		else
